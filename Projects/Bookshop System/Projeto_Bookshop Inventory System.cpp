...
class Bookshop{
	...
	public:
		...
		void define_file_books(std::string f_book); //V
		void define_file_login(std::string f_login); //V
		
		int open_write(int option); //V
		int open_read(int option); //V
		
		void close_write(); //V
		void close_read(); //V
		
		void new_book(Inventory book); //V
		void show_book(); //V
		
		void new_login(Login user); //V
		void show_login(); // Only for tests. I need to delete at the final version.
		...
		int sign_in(std::string email,std::string password); //V
		
		Inventory* search_name(Inventory *b,std::string book); //V
		Inventory* search_author(Inventory *b,std::string writer); //V
		Inventory* search_department(Inventory *b,std::string dep);	//V
		
		int delete_login(std::string login,std::string password); //V
		int delete_book(std::string book); //V
		
		int purchase_book(std::string name,int amount); //V
		
		Inventory* p_start_book(); //V
		
		int edit_book(Inventory search,Inventory new_b); //V
		int edit_login(Login search,Login new_login); //X
		
		//Adicionar funções para alterar informações da estrutura de dados ou 
		//para apagar	
};
